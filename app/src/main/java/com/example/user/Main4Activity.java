...
public class Main4Activity extends AppCompatActivity {
    ...
    private  final int[][] a = new int[8][8];//棋盤狀態
    ...
    private static boolean black_or_white=false; // true為黑子，false為白子

    private int mobility = 0;
    private int mobility_point = 0;//行動力
    ...
    private static final int search_level = 5;//搜尋層數、搜尋深度(奇數)

    private static int temp_x = 0;
    private static int temp_y = 0;//x與y座標暫存
        ...
        if(Degree_of_difficulty!=0 && check())
        {
            ...
            if(action==MotionEvent.ACTION_DOWN && x>=40 && x<1040 && y>=0 && y<1000)
            {
                if(a[y/125][(x - 40)/125]==0)
                {
                    ...
                    showToast("不能下在這裡!");
                //Toast.makeText(Main4Activity.this, "不能下在這裡!", Toast.LENGTH_SHORT).show();
            }
    ...
    private void chess()
    {
        ...
        if(change(y/125,(x - 40)/125))
        {
            ...
            count++;
            if(Degree_of_difficulty==3) //確認難易度
            {
                if(check())
                {
                    thread.start();
    ...
    {
        boolean lazi = false;//判斷是否能落子(有無違反規則等)
    ...
    private void hardAI()
    {
        ...
        //temp_x = current_board.getX();
        //temp_y = current_board.getY();
    ...
    private void hard(int aa, int bb) {
        ...
        int maxPoint = -200;//記錄各層最大得分

        int playerPoint = -161, aiPoint = -161 ;

        int initializationPoint;//各層初始分數

        int floorTempPoint = -200;//暫時存放該行動估計得分

        initializationPoint = temp_point;//紀錄各層初始分數

        int temp_mobility_point;//存放行動力得分

        Board best_decision = new Board();//暫存各層級最佳解

        Board tempBoard = new Board();//暫存運算的版面

        Board initializationBoard = new Board();//紀錄初始版面

        initializationBoard.board_record(a);//紀錄各層初始版面

        if (black_or_white) {//黑子
            if (search < search_level) { //必小於一個奇數值
                        ...
                        if (i - 1 >= 0)
                            if (a[i - 1][bb] == 0) {
                                if (square_point[i - 1][bb] > playerPoint) {
                                    playerPoint = square_point[i - 1][bb];//設定參照分數
                                    aaa = i - 1;
                                    bbb = bb;
                                    hard_change(aaa, bbb);
                                    tempBoard.board_record(a);//紀錄變動後版面

                                    checkMobility();//計算行動力
                                    temp_mobility_point = mobility_point;

                                    floorTempPoint = playerPoint - temp_mobility_point;//存放暫時估計得分
                        ...
                        if (i + 1 < 8)
                            if (a[i + 1][bb] == 0) {
                                if (square_point[i + 1][bb] > playerPoint) {
                                    ...
                                    if(playerPoint - temp_mobility_point > floorTempPoint) {//如果小於之後也不用計算

                                        floorTempPoint = playerPoint - temp_mobility_point;//變更暫估最大值
    ...
    public void winner(int black , int white)
    {
        if(black>white)
            showToast("Black WIN!");
        //Toast.makeText(Main4Activity.this, "Black WIN!", Toast.LENGTH_SHORT).show();
        if(black<white)
            showToast("White WIN!");
        //Toast.makeText(Main4Activity.this, "White WIN!", Toast.LENGTH_SHORT).show();
        if(black==white)
            showToast("DUEL!");
        //Toast.makeText(Main4Activity.this, "DUEL!", Toast.LENGTH_SHORT).show();
    }

    public void nowhere()
    {
        showToast("無地方下!換人");
        //Toast.makeText(Main4Activity.this, "無地方下!換人", Toast.LENGTH_SHORT).show();
    }
